    ...
    public static class Csverify
    {
        ...
        private static int Main(string[] args)
        {
            ...
            var optionset = new OptionSet()
                ...
                .Add("q|quiet", "Enable quiet mode. In quiet mode, only the resulting Viper code or error messages are shown.", option => quiet = option != null)
                .Add("r|reference=", "Adds the {ASSEMBLY.DLL} file as a reference when doing semantic analysis on the code. mscorlib and Soothsharp.Contracts are added automatically.", filename => references.Add(filename))
                .Add("a|assume=", "Translates the file {CLASS.CS} to Viper and prepends it to the main generated file, but its methods and functions won't be verified - their postconditions will be assumed to be true. ", filename => assumedFiles.Add(filename))   
                ...
                .Add("o|output-file=", "Print the resulting Viper code into the {OUTPUT.VPR} file.", filename => outputSilverFile = filename)
                .Add("l|line-numbers", "Print line numbers before the Viper code", ln => lineNumbers = ln != null)
                .Add("s|silicon", "Use the Silicon backend to verify the Viper code.", option => useSilicon = option != null)
                .Add("c|carbon", "Use the Carbon backend to verify the Viper code.", option => useCarbon = option != null)
                ;
            try
            {
        ...
        private static void WriteHelp(OptionSet optionSet)
        {
            ...
            Console.WriteLine();
            Console.WriteLine("By default, no backend is used (only translation is run).");
        }
            ...
            List<string> references)
        {
            ...
            if (useSilicon || useCarbon)
            {
                ...
                if (result.WasTranslationSuccessful)
                {
                    ...
                    else backend = new CarbonNailgunBackend(); // new CarbonBackend();
